...
public enum UnityChanStateType
{
    MOVEMENT = 0,
    ATTACK,
    RELOAD,
...
public class UnityChanState : FSMState
{
    protected UnityChanPlayer mPlayer = null;

    public UnityChanState(UnityChanPlayer player) 
        : base()
    {
        mPlayer = player;
        mFsm = mPlayer.mFsm;
...
public class UnityChanState_MOVEMENT : UnityChanState
{
    public UnityChanState_MOVEMENT(UnityChanPlayer player) : base(player)
    {
        mId = (int)(UnityChanStateType.MOVEMENT);
    }
    ...
    public override void Update()
    {
        ...
        for (int i = 0; i < mPlayer.mAttackButtons.Length; ++i)
        {
            if (mPlayer.mAttackButtons[i])
            {
                ...
                {
                    UnityChanState_ATTACK attack =
                  (UnityChanState_ATTACK)mFsm.GetState(
                            (int)UnityChanStateType.ATTACK);
...
public class UnityChanState_ATTACK : UnityChanState
{
    ...
    public UnityChanState_ATTACK(UnityChanPlayer player) : base(player)
    {
        mId = (int)(UnityChanStateType.ATTACK);
    }
    ...
    public override void Update()
    {
        ...
        //{
        //    mPlayer.mFsm.SetCurrentState((int)UnityChanStateType.RELOAD);
        //    return;
        //}

        //if (mPlayer.mAmunitionCount <= 0 && mPlayer.mBulletsInMagazine <= 0)
        //{
        //    mPlayer.mFsm.SetCurrentState((int)UnityChanStateType.MOVEMENT);
        //    mPlayer.NoAmmo();
        //    return;
        //}
...
public class UnityChanState_RELOAD : UnityChanState
{
    public float ReloadTime = 3.0f;
    float dt = 0.0f;
    public int previousState;
    public UnityChanState_RELOAD(UnityChanPlayer player) : base(player)
    {
